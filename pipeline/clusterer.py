...
def cluster_embeddings(embeddings: list[list[float]]) -> list[int]:
    ...
    # Try HDBSCAN first with params scaled to dataset size
    min_size = max(5, n // 15)
    ...
    # If too many clusters, tighten
    if n_clusters > 12:
        clusterer = HDBSCAN(min_cluster_size=min_size + 5, min_samples=5, metric="euclidean")
        labels = clusterer.fit_predict(X)
        n_clusters = len(set(labels)) - (1 if -1 in labels else 0)
        print(f"[Clusterer] Tightened to {n_clusters} clusters")
...
def name_clusters(reviews_by_cluster: dict[int, list[str]]) -> dict[int, str]:
    ...
    for cluster_id, texts in reviews_by_cluster.items():
        if cluster_id == -1:
